...
export const PROJECTS = [
  {
    ...
    content: {
      en: {
        ...
        contributions: [
          'Built responsive customer-facing flows for destination discovery, vessel search, filtering, and booking review.',
          'Structured price, deposit, and mandatory-extra information so customers can understand the booking total before continuing.',
          'Implemented the Next.js application around PostgreSQL and Prisma ORM and prepared it for VPS deployment.',
        ],
        outcome: 'The delivered platform connects search-focused destination pages with a practical charter search and booking flow. Visitors can move from researching a Croatian destination to filtering vessels and reviewing reservation costs without leaving the product experience.',
        screenshots: [
      ...
      ua: {
        ...
        contributions: [
          'Реалізував адаптивні клієнтські сценарії для вибору напрямку, пошуку, фільтрації суден і перевірки бронювання.',
          'Структурував інформацію про ціну, депозит та обов’язкові доплати, щоб користувач бачив складові вартості до продовження.',
          'Побудував застосунок на Next.js із PostgreSQL і Prisma ORM та підготував його до розгортання на VPS.',
        ],
        outcome: 'Готова платформа поєднує пошукові сторінки напрямків із практичним сценарієм вибору та бронювання чартеру. Відвідувач може перейти від вивчення регіону до фільтрації суден і перевірки вартості в межах одного продукту.',
        screenshots: [
          { src: '/projects/sea-travel/homepage.jpg', alt: 'Головна сторінка Sea Travel для оренди яхт і човнів у Хорватії', caption: 'Адаптивна головна сторінка з пошуком чартерів, пропозиціями напрямків, FAQ, соціальним контентом і навігацією послуг.' },
  ...
  },
  {
    ...
    content: {
      en: {
        ...
        outcome: 'The delivered frontend turns the approved visual direction into a responsive analytics interface with reusable patterns for wallet information, epoch progress, performance metrics, and bot status. The case is intentionally limited to the frontend contribution shown in the available project screen.',
        screenshots: [
      ...
      ua: {
        ...
        outcome: 'Реалізований frontend перетворює затверджений візуальний напрям на адаптивний аналітичний інтерфейс із повторно використовуваними патернами для wallet-даних, прогресу epoch, метрик ефективності та статусів ботів. Кейс свідомо обмежений показаним frontend-внеском.',
        screenshots: [
  ...
  },
  {
    ...
    content: {
      en: {
        ...
        outcome: 'The configurator replaces an error-prone manual description with a guided product-building flow. Customers can upload photos, choose a layout and product shape, adjust text and date settings, and review the configured result before moving toward an order.',
        screenshots: [
      ...
      ua: {
        ...
        outcome: 'Конфігуратор замінює неточний ручний опис керованим сценарієм створення товару. Покупець може завантажити фото, обрати макет і форму виробу, налаштувати текст та дату й перевірити результат перед переходом до замовлення.',
        screenshots: [
