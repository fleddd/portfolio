...
const projectCard = ({ name, description, stack, link }: IProjectCard) => {
	return (
		<div
			className={
				'flex flex-col justify-between min-w-72 max-w-72 min-h-72 bg-white-secondary dark:bg-black-secondary gap-5 p-3 pt-5 rounded-xl text-xs'
			}
		>
			<div className={'flex flex-col gap-5'}>
				<div className={'font-bold text-lg '}>{name}</div>
				<div className={'text-start text-gray-600 dark:text-gray-400 font-medium'}>
					{description}
				</div>
				<div className={'space-y-1'}>
					...
					<div className={'flex gap-2  flex-wrap  text-black cursor-pointer '}>
						{stack.map(item => (
							<motion.span
								className={'border-2 border-black dark:border-white dark:text-white rounded-xl px-2 py-1'}
